...
from config import *  # load_config, save_config, ensure_dependencies, MARKER_FILE, DATA_DIR, SCRIPT_DIR, etc.
...
def get_novium_dir():
    """Get the Novium installation directory (where the scripts live)."""
    return SCRIPT_DIR
...
def collect_removal_targets():
    ...
    # Temp files (now in DATA_DIR)
    novium_tmp = DATA_DIR / 'novium_tmp.txt'
    ...
    # Data directory (external configs, logs, version)
    if DATA_DIR.exists():
        targets.append(('directory', DATA_DIR))
    
    # Also check DATA_DIR items individually (they may exist even if dir was deleted)
    for name in ['config.json', '.version', '.novium_setup_done', 'novium_network.log', 'novium_tmp.txt']:
        f = DATA_DIR / name
        if f.exists():
            targets.append(('file', f))
